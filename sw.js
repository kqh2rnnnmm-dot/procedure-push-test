'use strict';
importScripts('./db.js');
const CACHE = 'procedure-push-shell-0.0.1';
...
self.addEventListener('push', event => {
  event.waitUntil((async () => {
    ...
    await self.registration.showNotification('Тест напоминания', {
      ...
      data:{id}, silent:false
    });
    if (id) {
      try { const old=(await TestDB.list()).find(x => x.id === id);if (!old?.receivedAt) await TestDB.patch(id,{receivedAt,dueAt:data.dueAt,shownAt:Date.now()}); } catch {}
...
self.addEventListener('notificationclick', event => {
  ...
  event.waitUntil((async () => {
    ...
    const target=new URL('./',self.registration.scope);target.searchParams.set('test',id);
    const clients=await self.clients.matchAll({type:'window',includeUncontrolled:true});
    const existing=clients.find(client => client.url.startsWith(self.registration.scope));
    if (existing) {
      existing.postMessage({type:'PUSH_OPENED',id});await existing.focus();
